    ...
    switch (val.type) {
      case "text":
        ...
        if (e.isPrivate&& e.msg.indexOf("ltoken") == -1 && !e.isMaster && e.msg.indexOf("authkey_ver") ==
          -1 && e.msg.indexOf("体力") == -1 && e.msg.indexOf("#绑定") == -1 &&e.msg.indexOf(/(\d+)/g)==-1 ){
          return true;
        }
        break;
      case "image":
      ...
      case "file":
        ...
        if (e.isPrivate && val.name.indexOf("txt") == -1) {
          return true;
        }
        break;
    }
...
async function dealIncrease(e) {
  ...
  if (e.user_id == BotConfig.account.qq) {
    ...
    if (Array.from(gl).length <= 10 && !hasMaster && BotConfig.account?.autoQuit == 100) {
      e.group.quit();
    }
    return;
  }
