...
def remake(name,qid):
    ...
    pic_list = []
    mes_list = []
    Life.load(join(FILE_PATH,'../data/data'))
    while True:
        life = Life()
    ...
    #name = ev["sender"]['card'] or ev["sender"]["nickname"]本次重生的基本信息如下：\n\n
    choice = 0
    person = name + "【你的天赋】\n"
    #print(life.talent.talents)
    ...
    pic_list.append(ImgText("\n这是"+name+"本次轮回的基础属性和天赋:").draw_title())
    ...
    pic_list.append(ImgText("这是"+name+"本次轮回的生平:").draw_title())
    pic_list.append(ImgText(mes).draw_text())

    sum = life.property.gensummary() #你的命运之轮到头了
    #pic_list.append("这是" + name + "本次轮回的评价:")
    pic_list.append(ImgText("这是" + name + "本次轮回的评价:").draw_title())
    pic_list.append(ImgText(sum).draw_resoult())
    n=0
