...
def generate_day_schedule(server='cn'):
    events = get_events(server, 0, 100)
    ...
    if len(events) == 0:
        draw_item(im, 1, 1, '无数据', 0,0,0,0, False)
    i = 1
    for event in events:
        ...
        if event['start_seconds'] <0 :
            ...
            draw_item(im, i, event['type'], event['title'],
                      event['left_days'],event['left_hours'],event['left_minutes'],event['left_seconds'], event['forever'])
            i += 1
    if has_prediction:
        ...
        for event in events:
            if event['start_seconds'] > 0:
                i += 1
                draw_item(im, i, event['type'], event['title'], -
                          event['start_days'],event['start_hours'],event['start_minutes'],event['start_seconds'], event['forever'])
    return im
