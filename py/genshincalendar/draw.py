...
font_path = os.path.join(os.path.dirname(__file__),'../../resrouces/fonts', 'wqy-microhei.ttc')
font = ImageFont.truetype(font_path, 20)

width = 1000

color = [
    {'front': 'black', 'back': (255,255,0)},
    {'front': 'black', 'back': (0, 255, 0)},
    {'front': 'white', 'back': (255,50,0)},
    {'front': 'black', 'back': (0,255,255)},
]
...
def draw_item(im, n, t, text, days,hours,minutes,seconds, forever):
    if t >= len(color):
        t = 1
    x = 0
    y = n * 30
    ...
    #print(im, n, t, text, days,hours,minutes,seconds)
    if seconds < 0:
        if forever:
            text1 = '永久开放'
        else:
            text1 = f'{days}天{hours}小时{minutes}分钟{-seconds}秒后结束'
    elif seconds >= 0:
        if forever:
            text1 = '即将永久开放'
        else:
            text1 = f'{-days}天{hours}小时{minutes}分钟{seconds}秒后开始'
