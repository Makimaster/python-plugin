...
def load_event_cn():
    ...
    if result and 'retcode' in result and result['retcode'] == 0 and detail_result and 'retcode' in detail_result and detail_result['retcode'] == 0:
        ...
        event_data['cn'].append({
            'title': '「2.7版本」',
            'start': datetime.strptime(
                curmon.strftime("%Y/5/25 11:00"), r"%Y/%m/%d %H:%M"),
            'end': datetime.strptime(
                curmon.strftime("%Y/7/18 11:00"), r"%Y/%m/%d %H:%M"),
            'forever': False,
            'type': 0
        })
        event_data['cn'].append({
            'title': '「2.7前瞻直播」',
            'start': datetime.strptime(
                curmon.strftime("%Y/5/20 20:00"), r"%Y/%m/%d %H:%M"),
            'end': datetime.strptime(
                curmon.strftime("%Y/7/18 11:00"), r"%Y/%m/%d %H:%M"),
            'forever': False,
            'type': 0
...
def get_events(server, offset, days):
    ...
    for event in event_data[server]:
        if end > event['start'] and start < event['end']:  # 在指定时间段内 已开始 且 未结束
            ...
            start1=start+timedelta(days=event['start_days'])
            end1=start+timedelta(days=event['left_days'])
            ...
            start1=start1+timedelta(hours=event['start_hours'])
            end1=end1+timedelta(hours=event['left_hours'])
            ...
            start1=start1+timedelta(minutes=event['start_minutes'])
            end1=end1+timedelta(minutes=event['left_minutes'])
