...
fd =os.path.join(FILE_PATH1,"../../resrouces")
...
def draw_pic(chain):
    ...
    width=398*2+30
    hight=795*2+400
    font_path = os.path.join(fd, "fonts","msyh.ttf")
    title_font = ImageFont.truetype(font_path, 20)
    im = Image.new("RGB", (width, hight), COLOR_WRITE)
    ...
    for i in chain:
        ...
        for j in i[0]:
            if j=='\n':
                pen.text((x1, y1), text, fill="#8d7650ff", font=title_font)
                text=''
                y1=y1+25
            else:
                text+=j
        ...
        if n==1:
            ...
            y=ymax+10
            y1=ymax+10+795+10
    im=im.crop((0,0,width,ymax+10))
    im.save(fd+'/tarot/torot.png')
    
        
            
    

def chain_reply(chain, msg, image):
    msg = "".join(msg)
...
if __name__=="__main__":
    ...
    for count in range(4):
        ...
        img_path = os.path.join(fd, "tarot", f"{card_key}.jpg")
        image_file = f"{img_path}"

        # 特殊规则：愚者有两张
        if card_key == '愚者':
            rand = randint(1, 2)
            rand_path = os.path.join(fd, "tarot", f"{card_key}{rand}.jpg")
        ...
        msg = []#meaning_key,"，",
        msg.extend([meaning_value,"\n",card_key,"，",card_value,"\n"])
        if count < 4:
            chain_reply(chain, msg, image_file)
