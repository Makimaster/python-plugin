...
FZSJ_QINGCRJ="FZSJ-QINGCRJ.ttf"
FILE_PATH1 = os.path.dirname(__file__)
data_path = os.path.join(FILE_PATH1,"../../resrouces/images")
...
def draw_text(
    ...
    emoji_font_file = BytesIO(get_font(EMOJI_FONT))
...
def save_gif(frames: List[IMG], duration: float) -> BytesIO:
    #output = BytesIO()
    #frames.save(data_path+'/123.gif')
    print('gif')
    imageio.mimsave(data_path+'/123.gif', frames, format="gif", duration=duration)
    return 0
...
def make_jpg_or_gif(
    img: IMG, func: Maker, gif_zoom: float = 1, gif_max_frames: int = 30
) -> BytesIO:
    """
    制作静图或者动图
    :params
...
def to_image1(data: bytes, convert: bool = True) -> IMG:
    image = Image.open(BytesIO(data))
    if convert:
        image = to_jpg(image).convert("RGBA")
    return image

def to_image2(data: bytes, convert: bool = True) -> IMG:
    image = Image.open(BytesIO(data))
    return image

def load_image(name: str) -> IMG:
    image = get_image(name)
    return Image.open(BytesIO(image)).convert("RGBA")

def load_image1(name: str) -> IMG:
    return Image.open(data_path+'/'+name)
...
def help_image(commands) -> BytesIO:
    ...
    return img.save(data_path+"/help.jpeg")
