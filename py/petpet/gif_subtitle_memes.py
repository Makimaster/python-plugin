...
def make_gif(
    ...
    img = load_image1(f"gif/{filename}")
    
    frames=[]
    #save_gif(img.tell,img.info["duration"]/1000)
    ...
    for part, text in zip(parts, texts):
        for frame in part:
            try:
                fontsize =fit_font_size(text, frame.width - 10,frame.height-10 , fontname, 40, 10)
                font = load_font(fontname, fontsize)
                draw_text(
                    frame,
                    (padding_x, frame.height-fontsize-padding_y, frame.width - padding_x, frame.height-padding_y),
                    text,
                    font=font,
                    fill="white",
                )
    ...
    return save_gif([frame for frame in frames], img.info["duration"] / 4000)
