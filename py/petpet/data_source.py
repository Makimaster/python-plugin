...
def download_image(user: UserInfo, convert: bool = True):
    img = None
    if user.qq:
        img =download_avatar(user.qq)
    elif user.img_url:
        img =download_url(user.img_url)
...
def make_image(
    ...
    download_image(sender, command.convert)
    for user in users:
        download_image(user, command.convert)
    return command.func(users, sender=sender, args=args)
